...
class ComplexSVRegionGroupGenerator(module.Module):
    ...
    def _write_group(self, group):
        ...
        with open(out_prefix + '.sv', 'w') as sv_f, open(out_prefix + '.region', 'w') as region_f, open(out_prefix + '.enhancer', 'w') as enhancer_f, open(out_prefix + '.super_enhancer', 'w') as super_enhancer_f:
            for region in group.region_list:
    ...
    def _group_sv(self, linkage_distance=1000000.0):  # 1M
        for i, sv_re in enumerate(self.in_data):
            if sv_re.chrom_5p not in constants.chrs or sv_re.chrom_3p not in constants.chrs:
                continue
            if sv_re.chrom_5p == sv_re.chrom_3p and \
                    abs(sv_re.bkpos_5p - sv_re.bkpos_3p) < 1000:
                continue
            # print(sv.chrom_5p, sv.bkpos_5p/linkage_distance, sv.chrom_3p, sv.bkpos_3p/linkage_distance)
            self._append_sv_to_groups(sv_re)
